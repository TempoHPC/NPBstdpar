...
c---------------------------------------------------------------------
       ...
       do concurrent (k = 0:grid_points(3)-1, j = 0:grid_points(2)-1,
     >                i = 0:grid_points(1)-1) local(rho_inv, aux)
...
c---------------------------------------------------------------------
       ...
       do concurrent (k = 0:nz2+1, j = 0:ny2+1, i = 0:nx2+1)
     >    local(m)
                do    m = 1, 5
...
c---------------------------------------------------------------------
       do concurrent (k = 1:nz2, j = 1:ny2, m = 1:5)
                rhs(m,1,j,k) = rhs(m,1,j,k)- dssp * 
     >                    ( 5.0d0*u(m,1,j,k) - 4.0d0*u(m,2,j,k) +
     >                            u(m,3,j,k))
       end do

       do concurrent (k = 1:nz2, j = 1:ny2, m = 1:5)
                rhs(m,2,j,k) = rhs(m,2,j,k) - dssp * 
     >                    (-4.0d0*u(m,1,j,k) + 6.0d0*u(m,2,j,k) -
     >                      4.0d0*u(m,3,j,k) + u(m,4,j,k))
       end do

       do concurrent (k = 1:nz2, j = 1:ny2, i = 3:nx2-2, m = 1:5)
     ...
     >                         u(m,i+2,j,k) )
       ...
       do concurrent (k = 1:nz2, j = 1:ny2, m = 1:5)
                rhs(m,nx2-1,j,k) = rhs(m,nx2-1,j,k) - dssp *
     >                    ( u(m,nx2-3,j,k) - 4.0d0*u(m,nx2-2,j,k) + 
     >                      6.0d0*u(m,nx2-1,j,k) - 4.0d0*u(m,nx2,j,k) )
       end do

       do concurrent (k = 1:nz2, j = 1:ny2, m = 1:5)
                rhs(m,nx2,j,k) = rhs(m,nx2,j,k) - dssp *
     >                    ( u(m,nx2-2,j,k) - 4.d0*u(m,nx2-1,j,k) +
     >                      5.d0*u(m,nx2,j,k) )
       end do
...
c---------------------------------------------------------------------
       do concurrent (k = 1:nz2, i = 1:nx2, m = 1:5)
                rhs(m,i,1,k) = rhs(m,i,1,k)- dssp * 
     >                    ( 5.0d0*u(m,i,1,k) - 4.0d0*u(m,i,2,k) +
     >                            u(m,i,3,k))
       end do

       do concurrent (k = 1:nz2, i = 1:nx2, m = 1:5)
                rhs(m,i,2,k) = rhs(m,i,2,k) - dssp * 
     >                    (-4.0d0*u(m,i,1,k) + 6.0d0*u(m,i,2,k) -
     >                      4.0d0*u(m,i,3,k) + u(m,i,4,k))
       end do

       do concurrent (k = 1:nz2, j = 3:ny2-2, i = 1:nx2, m = 1:5)
                   rhs(m,i,j,k) = rhs(m,i,j,k) - dssp * 
     ...
     >                         u(m,i,j+2,k) )
       ...
       do concurrent (k = 1:nz2, i = 1:nx2, m = 1:5)
                rhs(m,i,ny2-1,k) = rhs(m,i,ny2-1,k) - dssp *
     >                    ( u(m,i,ny2-3,k) - 4.0d0*u(m,i,ny2-2,k) + 
     >                      6.0d0*u(m,i,ny2-1,k) - 4.0d0*u(m,i,ny2,k) )
       end do

       do concurrent (k = 1:nz2, i = 1:nx2, m = 1:5)
                rhs(m,i,ny2,k) = rhs(m,i,ny2,k) - dssp *
     >                    ( u(m,i,ny2-2,k) - 4.d0*u(m,i,ny2-1,k) +
     >                      5.d0*u(m,i,ny2,k) )
       end do
...
c---------------------------------------------------------------------
       do concurrent (j = 1:ny2, i = 1:nx2, m = 1:5)
                rhs(m,i,j,1) = rhs(m,i,j,1)- dssp * 
     >                    ( 5.0d0*u(m,i,j,1) - 4.0d0*u(m,i,j,2) +
     >                            u(m,i,j,3))
       end do

       do concurrent (j = 1:ny2, i = 1:nx2, m = 1:5)
                rhs(m,i,j,2) = rhs(m,i,j,2) - dssp * 
     >                    (-4.0d0*u(m,i,j,1) + 6.0d0*u(m,i,j,2) -
     >                      4.0d0*u(m,i,j,3) + u(m,i,j,4))
       end do

       do concurrent (k = 3:nz2-2, j = 1:ny2, i = 1:nx2, m = 1:5)
                   rhs(m,i,j,k) = rhs(m,i,j,k) - dssp * 
     ...
     >                         u(m,i,j,k+2) )
       ...
       do concurrent (j = 1:ny2, i = 1:nx2, m = 1:5)
                rhs(m,i,j,nz2-1) = rhs(m,i,j,nz2-1) - dssp *
     >                    ( u(m,i,j,nz2-3) - 4.0d0*u(m,i,j,nz2-2) + 
     >                      6.0d0*u(m,i,j,nz2-1) - 4.0d0*u(m,i,j,nz2) )
       end do

       do concurrent (j = 1:ny2, i = 1:nx2, m = 1:5)
                rhs(m,i,j,nz2) = rhs(m,i,j,nz2) - dssp *
     >                    ( u(m,i,j,nz2-2) - 4.d0*u(m,i,j,nz2-1) +
     >                      5.d0*u(m,i,j,nz2) )
       end do
